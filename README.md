# Java Programming II - MOOC.fi (University of Helsinki)

![Java](https://img.shields.io/badge/java-%23ED8B00.svg?style=for-the-badge&logo=openjdk&logoColor=white)
![Git](https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white)
![Status](https://img.shields.io/badge/status-active-success.svg?style=for-the-badge)

## 🇺🇸 About the Repository
This repository contains my solutions for **Part 2** of the renowned **Java Programming MOOC** from the University of Helsinki. My goal is to practice daily and improve my skills in Object-Oriented Programming and Java fundamentals.

### 🚀 Key Concepts Learned
In this second part of the course, I am focusing on advanced topics such as:
* **Object-Oriented Programming (OOP):** Deep dive into inheritance, interfaces, and polymorphism.
* **Data Structures:** Using Lists, HashMaps, and understanding algorithmic complexity.
* **Functional Programming:** Introduction to Streams, Lambdas, and functional interfaces.
* **File Handling:** Reading and writing data.
* **Clean Code:** Writing readable, maintainable, and modular code.

---

## 🇧🇷 Sobre o Repositório
Este repositório contém minhas soluções para a **Parte 2** do renomado curso **Java Programming MOOC** da Universidade de Helsinque. Meu objetivo é praticar diariamente e aprimorar minhas habilidades em Programação Orientada a Objetos e nos fundamentos do Java.

### 🚀 Principais Conceitos Aprendidos
Nesta segunda parte do curso, o foco está em tópicos avançados como:
* **Programação Orientada a Objetos (POO):** Aprofundamento em herança, interfaces e polimorfismo.
* **Estruturas de Dados:** Uso de Listas, HashMaps e compreensão de complexidade de algoritmos.
* **Programação Funcional:** Introdução a Streams, Lambdas e interfaces funcionais.
* **Manipulação de Arquivos:** Leitura e escrita de dados.
* **Clean Code:** Escrita de código legível, manutenível e modular.

## 📂 Project Structure / Estrutura do Projeto
Each folder represents a specific exercise from the course, organized by sections.
*Cada pasta representa um exercício específico do curso, organizado por seções.*

## 🛠️ Tools Used / Ferramentas
* **Java 11+**
* **TMCBeans / IntelliJ IDEA**
* **JUnit** (for testing)
* **Git & GitHub**

---
<div align="center">
  <sub>Built with ☕ and focus by <a href="https://github.com/nickevangelista">Nick Evangelista</a></sub>
</div>
